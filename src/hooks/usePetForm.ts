...
export function usePetForm(props: Props) {
  ...
  useEffect(() => {
    ...
    methods.reset({
      img_url: pet.imgUrl || null,
      petName: pet.petName,
      petTypeId: petTypes.find((petType) => petType.name === pet.petType)?.id,
      sex: pet.sex,
      breed: pet.breed,
      dateOfBirth: pet.dateOfBirth ? new Date(pet.dateOfBirth) : null,
      color: pet.color,
  ...
  const handleSubmit = async (data: PetFormValues) => {
    ...
    try {
      if (props.mode === "create") {
        const createdPet = await petApi.createPet(data);
        ...
        console.log("usePetForm mode:", props.mode);
        console.log("redirectOnSuccess:", props.redirectOnSuccess);

        if (props.redirectOnSuccess !== false) {
          console.log("redirecting to /pets");
          router.push("/pets");
        }
        ...
        const updatedPet = await petApi.updatePet(props.petId, data);
