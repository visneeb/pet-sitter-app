...
export function ProgressBar({ currentStep, steps = defaultSteps, className }: Props) {
  return (
    <div
      className={cn(
        "w-full md:rounded-2xl bg-white md:px-10 py-4 shadow-sm flex px-0 ",
        className
      )}
      aria-label="Booking progress"
    >
      <ol className="flex gap-5 md:gap-12 w-full justify-center items-center">
        {steps.map((s, idx) => {
          ...
          return (
            <li key={s.key} className="flex ">
              <div className="flex items-center gap-3">
                <div
                  className={cn(
                    "flex h-8 w-8  justify-center rounded-full style-body-1 text-center items-center",
